...
class CartItem_inandiar {
  ...
  double get totalPrice_inandiar => product_inandiar.price_inandiar * quantity_inandiar;
}
...
class CartProvider_inandiar with ChangeNotifier {
  ...
  double get totalPrice_inandiar {
    return _items_inandiar.fold(0.0, (sum, item) => sum + item.totalPrice_inandiar);
  }

  void addToCart_inandiar(Product_inandiar product_inandiar) {
    final existingIndex_inandiar = _items_inandiar.indexWhere((item) => item.product_inandiar.id_inandiar == product_inandiar.id_inandiar);
    if (existingIndex_inandiar >= 0) {
      _items_inandiar[existingIndex_inandiar].quantity_inandiar++;
    } else {
      _items_inandiar.add(CartItem_inandiar(product_inandiar: product_inandiar));
    }
    notifyListeners();
    print('Added to cart: ${product_inandiar.name_inandiar}'); // Debug print biar tau
  }

  void removeFromCart_inandiar(String productId_inandiar) {
    _items_inandiar.removeWhere((item) => item.product_inandiar.id_inandiar == productId_inandiar);
    notifyListeners();
  ...
  Future<void> checkout_inandiar(String nim_inandiar) async {
    ...
    double finalTotal_inandiar = totalPrice_inandiar - discount_inandiar + shipping_inandiar;
    ...
    for (var item in _items_inandiar) {
      DocumentReference docRef_inandiar = FirebaseFirestore.instance.collection('products_inandiar').doc(item.product_inandiar.id_inandiar);
      batch_inandiar.update(docRef_inandiar, {'stock_inandiar': FieldValue.increment(-item.quantity_inandiar)});
    }
